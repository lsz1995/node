...
class NEM_spider(object):
    ...
    def from_playlist_get_song_list(self, playlist_id):#从歌单中获取歌曲列表
    ...
    def get_artists_songlist(self, artist_id):#获取歌手的音乐列表id
    ...
    def get_song_lyric(self, song_id):#获取歌词
    ...
    def get_song_comments(self, song_id, offset=0, total='false', limit=100): #获取歌曲评论
...
if __name__ == '__main__':
    ...
    lyric_list = []
    musicclist = spider.get_artists_songlist(6452)#获取新歌榜的歌曲信息
    for music in musicclist:

        lyric = spider.get_song_lyric(music['id'])
        # print(lyric)
        if lyric is not None:
            # pprint(lyric)
            lyric_list.append(lyric)
